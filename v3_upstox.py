"""
====================================================================
NSE INTRADAY 09:45 SHORT-BIASED SCANNER - V3 (UPSTOX DATA)
====================================================================
...
def get_upstox_token() -> str:
    ...
    if not token:
        ...
        print("Use the UPSTOX_ACCESS_TOKEN environment variable, or enter it now.")
...
def download_upstox_instruments() -> Dict[str, str]:
    ...
    # Try the daily cached uncompressed file first.
    ...
    # Download the official NSE instrument master when cache is missing.
    if records is None:
...
def download_intraday_batch(
    ...
    # One V3 historical request per stock gives the warm-up period and,
    # using one Upstox V3 historical-candle request per instrument.
    from_date = (
        target_date
...
def download_nifty_data(
    ...
    # Nifty 50 uses an index instrument key, so it is not part of the
    # NSE_EQ instrument map.
...
def select_mode() -> Tuple[
    ...
    print(
        "NSE 09:45 SHORT-BIASED INTRADAY SCANNER V3 - UPSTOX DATA"
    )
